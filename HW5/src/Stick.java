...
public class Stick implements Comparable<Stick> {
	...
	public Stick(String name, double x1, double y1, double z1, double x2, double y2, double z2){
		this.name = name;
		this.x1 = x1;
		this.y1 = y1;
		this.z1 = z1;
	...
	public int compareTo(Stick v){
		...
		// Step 5. Now that we know z for both sticks, we know their relationship
	...
	*/
	private void checkRelationship(Stick v){
		...
		// Set coefficients for a 2x2 matrix representing a system of linear equations: 
		//	 [ a.m  a.n ] [ x ] = [ a.b ]
		//	 [ b.m  b.n ] [ y ]   [ b.b ]
		...
		matrix[0][0] = this.m;	matrix[0][1] = v.m;	
		matrix[1][0] = this.n;		matrix[1][1] = v.n;
		vector[0][0] = this.b;
		vector[1][0] = v.b;
	    ...
	    if (det != 0){
	    	...
	    	setXandY(v, vector);
	    	System.out.println("Line segments intersect at " + this.x + " and " + this.y);

		    // Step 3. Check if intersection point is valid
		    // return 0 if the intersection point is beyond the stick/line segment of A or B
		    if (this.x < Math.min(this.x1,this.x2) || this.x > Math.max(this.x1,this.x2)) unrelated(v);
		    if (v.x < Math.min(v.x1,v.x2) || v.x > Math.max(v.x1,v.x2)) unrelated(v);

		    // Step 4. Find z using parametric equations of lines A and B:
		    // x = x1 + r (x2 - x1)
		    // y = y1 + r (y2 - y1)
		    // z = z1 + r (z2 - z1)
		    // Since we know x (and y) now, we can find r = (xi - x1) / (x2 - x1)
	...
	private void setCoefficients(Stick v){
		...
		v.b = v.y1 - v.m*(v.x1);
		v.n = 1;
		System.out.println("Stick " + v.name + ": " + v.m + "x + " + v.n + "y" + " = " + v.b);
	}
	
	private void setXandY(Stick v, double[][] vector){ // set intersection values
		v.x = vector[0][0];
		v.y = vector[1][0];
	}
