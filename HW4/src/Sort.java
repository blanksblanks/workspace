// add method for frame, takes various components from swing like
// java button and text field into the fame
// defaultcloseoperation (NEED to tell program that is)
// setVisible is important too
// Basically it automatically sizes the JFrame based on the size of components
// have to write onw action listener interface - when you add actionlistener,
// it means this widget will explicitly invoke
// Create two separate instances of GreetingListener, which take text field
// because we want to invoke that method
...
// Already talked a lot about heapsort
// Not talking about shell sort
...
public final class Sort {
	...
	// When you pass reference to array, no need to return a because
	// it's changes come out
	// Private method to prevent us from having to pass smaller arrays
	// every time we want to recursively pass around arrays
	/**
	 * Quicksort algorithm.
	 * 
	 * @param a
	...
	// ultimately is the base case - when it gets to that size you call
	// diff method. next helper is swap references method
	private static final int CUTOFF = 3;

	// Takes location of 1 object and swaps it with 2nd object
	/**
	...
	// smallest to first element, biggest to last element
	// we do a comparison between the center and the left elem
	private static <AnyType extends Comparable<? super AnyType>> AnyType median3(
			AnyType[] a, int left, int right) {
		int center = (left + right) / 2;
		if (a[center].compareTo(a[left]) < 0)
			swapReferences(a, left, center);
		if (a[right].compareTo(a[left]) < 0)// if right elem is smaller, swap
			swapReferences(a, left, right);
		if (a[right].compareTo(a[center]) < 0)
			swapReferences(a, center, right);

		// Place pivot at position right - 1
		swapReferences(a, center, right - 1); // right - 1 takes pivot and moves
												// it to pivot
		// that's the state of the array after returning median 3 calls
		return a[right - 1];
	}
	...
	private static <AnyType extends Comparable<? super AnyType>> void quicksort(
		...
		if (left + CUTOFF <= right) {
			// goal is to put midpoint in the second to last element
	...
	// if we get base case, we invoke insertionsort
	...
	public static void main(String[] args) {
		...
		Integer[] a = { 111, 142, 543, 123, 65, 111, 453, 879, 572, 434, 111, 242, 111, 811, 102 };
//		Integer[] a = { 1, 2, 3, 4, 1, 1, 4, 8 };

		System.out.println("Pre quicksort three way partition:");
		print(a);
...
// if (this.intValue > anotherInt) return 1;
// else if (this.intValue < anotherInt) return -1;
// else return 0;
// }